...
namespace Rest_API.Controllers.ControllerMethods
{
    public class RequestValidator : ControllerBase
    {
        ...
        public ApiResponse<TR> ValidateAndPerfom<T, TR>(T data, Func<T, ApiResponse<TR>> MethodName,
            HttpResponse recievedResponse) where TR : class
        {
            ...
            var apiResponse = MethodName(data);
        ...
        /// <param name="recievedResponse">The original HttpResponse from the calling controller.</param>
        public ApiResponse<TR> ValidateAndPerfom<TR>(Func<ApiResponse<TR>> MethodName, HttpResponse recievedResponse)
            where TR : class
        {
            var apiResponse = MethodName();
            recievedResponse.StatusCode = (int) apiResponse.Code;
            return apiResponse;
        }
    }
